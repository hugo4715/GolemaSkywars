...
public class CageMenuFirstPage extends VirtualMenu {

	private static Map<Player, Inventory> playerInventoryMap = new HashMap<Player, Inventory>();
	
	/**
	 * Constructeur du Menu de la page 1.
	 * 
	 * @param player
	 */
	public CageMenuFirstPage(Player player) {
		super(player, "Plate-forme � 1", 6);
		
		/**
		 * Cages.
		 */
		for(CagesInfo cagesInfo : CagesInfo.values()) {
			if(cagesInfo.getPage() == 1) {
				/*if((cagesInfo.getPermission() != null) && (!(GolemaPlayer.getGolemaPlayer(player).hasPermission(cagesInfo.getPermission())))) {
	...
	public void onInventoryClick(InventoryClickEvent event) {
		...
		if(event.getInventory().equals(playerInventoryMap.get((Player) event.getWhoClicked()))) {
			...
			if(event.getCurrentItem().getType().equals(Material.PAPER)) {
				if(event.getSlot() == 50) {
					new CageMenuSecondPage(player);
				} else {
					// Il n'y a pas de cage pr�c�dente ici.
				}
				return;
			}
			...
			if(event.getCurrentItem().getType().equals(Material.NETHER_STAR)) {
				...
				for(@SuppressWarnings("unused") CagesInfo cagesInfo : CagesInfo.values()) {
					/*if(GolemaPlayer.getGolemaPlayer(player).hasPermission(cagesInfo.getPermission())) {
